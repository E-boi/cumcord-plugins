...
const ConnectedPrivateChannelsList = findByDisplayName(
...
export default function () {
  ...
  this.injections.push(
    after("default", ConnectedPrivateChannelsList, (_, res) => {
      ...
      props.privateChannelIds = props.privateChannelIds.filter(
      ...
      categories.forEach((category) => {
        ...
        if (!category.collapsed) {
          const dms = category.dms
            ...
            .map((id) =>
              React.createElement(Channel, {
                channelId: id,
                selected: props.selectedChannelId === id,
              })
            );
          ...
          props.children.push(
            React.createElement(Channel, { channelId: dm, selected: true })
          );
        }
      });
      return res;
